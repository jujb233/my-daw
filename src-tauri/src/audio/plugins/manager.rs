...
impl PluginManager {
    ...
    fn scan_native_plugins(&mut self) {
        // First, try to scan a plugins directory for official/native plugins
        let plugin_dir = Path::new("plugins/official");
        if plugin_dir.exists() && plugin_dir.is_dir() {
            if let Ok(entries) = fs::read_dir(plugin_dir) {
                for entry in entries.flatten() {
                    let plugin_folder = entry.path();
                                ...
                                match lua.load(&content).eval::<mlua::Table>() {
                                    Ok(tbl) => {
                                        ...
                                        let backend_tbl: Option<mlua::Table> =
                                            tbl.get("backend").ok();
                                        if let (Some(id), Some(name), Some(backend)) =
                                            (id, name, backend_tbl)
                                        {
                                            let btype: Option<String> = backend.get("type").ok();
                                            if let Some(t) = btype {
                                                if t == "local" {
                                                    let lib_path_str: Option<String> =
                                                        backend.get("path").ok();
                                                    if let Some(bp) = lib_path_str {
                                                        // Resolve path relative to plugin folder
                                                        let full = plugin_folder.join(&bp);
                                                        self.local_paths.insert(
                                                            id.clone(),
                                                            full.to_string_lossy().to_string(),
                                                        );
                                                        // Try load plugin to get info
                                                        if let Ok(plugin) = unsafe {
                                                            crate::audio::core::ffi_plugin::FFIPlugin::new(self.local_paths.get(&id).unwrap(), 44100.0)
                                                        } {
                                                    ...
                                                    let module: Option<String> =
                                                        backend.get("module").ok();
                                                    if let Some(m) = module {
                                                        // map module to actual builtin plugin
                                                        if let Some(plugin) =
                                                            Self::create_builtin_by_module(&m)
                                                        {
                                                            let info = plugin.info();
                                                            // override unique id and name from manifest if provided
                                                            let mut info = info.clone();
                                                            info.unique_id = id.clone();
                                                            info.name = name.clone();
                                                            self.known_plugins
        ...
        // No builtin implementations are provided by the host; rely on manifests and external libs.
    }
    ...
    pub fn create_plugin(&self, unique_id: &str) -> Option<Box<dyn Plugin>> {
        ...
        if let Some(lib_path) = self.local_paths.get(unique_id) {
            unsafe {
                if let Ok(plugin) =
                    crate::audio::core::ffi_plugin::FFIPlugin::new(lib_path, 44100.0)
                {
                    return Some(Box::new(plugin));
                }
            }
