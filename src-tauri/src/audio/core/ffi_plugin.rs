...
impl FFIPlugin {
    pub unsafe fn new(lib_path: &str, sample_rate: f32) -> Result<Self, anyhow::Error> {
        ...
        let destroy: Symbol<unsafe extern "C" fn(*mut c_void)> =
            unsafe { lib_ref.get(b"destroy_plugin")? };
        let process: Symbol<unsafe extern "C" fn(*mut c_void, *mut f32, usize, usize)> =
            unsafe { lib_ref.get(b"plugin_process")? };
        let set_param: Symbol<unsafe extern "C" fn(*mut c_void, u32, f32)> =
            unsafe { lib_ref.get(b"plugin_set_param")? };
        let get_param: Symbol<unsafe extern "C" fn(*mut c_void, u32) -> f32> =
            unsafe { lib_ref.get(b"plugin_get_param")? };
        let info: Symbol<unsafe extern "C" fn() -> *mut c_char> =
            unsafe { lib_ref.get(b"plugin_info_json")? };
        ...
        let state_get = match unsafe {
            lib_ref.get::<unsafe extern "C" fn(*mut c_void, *mut usize) -> *mut u8>(
                b"plugin_get_state",
            )
        } {
            Ok(s) => Some(s),
        ...
        let state_free = match unsafe {
            lib_ref.get::<unsafe extern "C" fn(*mut u8, usize)>(b"plugin_free_state_blob")
        } {
            Ok(s) => Some(s),
            Err(_) => None,
        };
...
impl Plugin for FFIPlugin {
    ...
    fn get_param(&self, id: u32) -> f32 {
        unsafe { (self.get_param_fn)(self.inst, id) }
    }

    fn set_param(&mut self, id: u32, value: f32) {
        unsafe { (self.set_param_fn)(self.inst, id, value) }
    }
}
