...
export const PianoRoll: Component<PianoRollProps> = props => {
        ...
        const cursorPosition = () => {
                ...
                const clipStart = defaultTimeService.ticksToSeconds(defaultTimeService.positionToTicks(c.start))
        ...
        const handleNoteUpdate = (noteId: string, newStartPx: number, newWidthPx: number, newNoteVal?: number) => {
                const c = clip()
                if (!c) return
                                ...
                                >
                                        ...
                                        <Show when={clip()}>
                                                <For each={clip()!.notes}>
                                                        {note => {
                                                                const startPx =
                                                                        defaultTimeService.positionToTicks(note.start) *
                                                                        pixelsPerTick()
                                                                const widthPx =
                                                                        note.duration.totalTicks * pixelsPerTick()
