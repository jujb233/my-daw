...
const TrackRow: Component<TrackRowProps> = props => {
    ...
    return (
        <div class='flex h-24 border-b border-outline-variant bg-surface-container-low shrink-0'>
            ...
            {/* Timeline Area */}
            <div
                class='relative bg-surface-container-lowest overflow-hidden'
                style={{ width: `${TOTAL_BARS * PX_PER_BAR}px` }}
            >
                {/* Grid Lines */}
                <div
                            ...
                            <GridClip
                                ...
                                onUpdate={updates => {
                                    if (updates.left !== undefined) {
                                        const newStartBar =
                                            Math.round(updates.left / PX_PER_BAR) + 1
                                        import('../../store').then(m =>
                                            m.updateClipPosition(clip.id, newStartBar)
                                        )
                                    }
                                }}
    ...
    return (
        <div class='flex-1 flex flex-col overflow-hidden bg-surface'>
            ...
            <div class='flex h-8 bg-surface-container-high border-b border-outline-variant shrink-0'>
                ...
                {/* Ruler */}
                <div ref={rulerContainer} class='flex-1 overflow-hidden whitespace-nowrap relative'>
            ...
            >
                <div class='min-w-fit'>
                    <For each={store.tracks}>
                        {track => <TrackRow name={track.name} trackId={track.id} scrollLeft={0} />}
                    </For>
