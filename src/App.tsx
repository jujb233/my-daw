...
export default function App() {
        ...
        onMount(() => {
                // show project chooser first; when done we'll fetch state
                // fetchInstances/fetch* will be called after user selects project
        })

        const [chooserDone, setChooserDone] = createSignal(false)

        const onChooserDone = async () => {
                setChooserDone(true)
                // now fetch runtime state
        ...
        return (
                <div class='bg-background text-on-background flex h-screen w-screen flex-col overflow-hidden'>
                        ...
                        <Show when={!chooserDone()}>
                                <ProjectChooser onDone={onChooserDone} />
                        </Show>
                        {/* Bottom Bar */}
                        <BottomEditor />
                        {/* Settings Modal */}
