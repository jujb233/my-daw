...
export const DawService = {
    async addClip(name: string, startTime: number, duration: number, instrumentIds: number[] = []): Promise<number> {
    ...
    async updateClip(id: number, updates: Partial<BackendClip>): Promise<void> {
        ...
        if (updates.instrument_routes !== undefined) args.instrumentRoutes = updates.instrument_routes;
